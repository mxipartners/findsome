...
Projects.allow({
  update: function(userId, project) { return isProjectMember(userId, project); },
  remove: function(userId, project) { return isProjectMember(userId, project); },
});
...
Projects.deny({
  update: function(userId, project, fieldNames) {
    ...
    return (_.without(fieldNames, 'title', 'description',
      'measuresCount', 'risksCount', 'findingsCount', 'sourcesCount',
      'members').length > 0);
  }
});
...
validateProject = function (project) {
  ...
  if (!project.members)
    errors.members = "Please select at least one project member";
